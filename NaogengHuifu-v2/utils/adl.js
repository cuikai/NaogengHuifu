 ...
 * 分成两档：
 *   STOP  停下来休息、今天不练了
 *   CALL  停下来还要去医院 —— 这一档原来完全没有，
 *         「胸痛」和「练完有点累」被写在同一个清单里，等于没写。
 */
const STOP = [
  { k: '疼痛',                       v: '立刻停，不要「忍一下就过去了」' },
  { k: '头晕、乏力、气短、脸色发白', v: '停下来休息，坐好，别急着再站起来' },
  { k: '憋气',                       v: '全程不要憋气，慢慢做，不要甩、不要用猛劲' },
  { k: '第二天比平时更累',           v: '说明昨天做多了，会自动给你减量' }
];

/** 这一档不是「休息一下」，是「今天别练了，去看医生」 */
const CALL = [
  { k: '胸口压着疼、闷得喘不上气', v: '立刻停下坐好；不缓解就打 120' },
  { k: '突然脸歪、一只手抬不起来、说话说不清、天旋地转看不清', v: '打 120，记住是几点开始的' },
  { k: '一条腿突然肿、发烫、按着疼', v: '当天停练，去医院排一下血栓；不要揉那条腿' },
  { k: '摔了一跤（尤其撞到头、或在吃抗凝药）', v: '当天去医院看过再练' }
];

module.exports = {
  forLevel: forLevel, noteFor: noteFor, BY_LEVEL: BY_LEVEL,
  STOP: STOP, CALL: CALL
};
